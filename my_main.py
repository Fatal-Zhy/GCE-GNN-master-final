...
parser.add_argument('--dataset', default='Beauty', help='Beauty/Cell')
...
parser.add_argument('--dropout_gcn', type=float, default=0.4, help='Dropout rate.')       # [0, 0.2, 0.4, 0.6, 0.8]
parser.add_argument('--dropout_local', type=float, default=0.5, help='Dropout rate.')     # [0, 0.5]
parser.add_argument('--dropout_global', type=float, default=0.3, help='Dropout rate.')
parser.add_argument('--validation', action='store_true', help='validation')
parser.add_argument('--valid_portion', type=float, default=0.1, help='split the portion')
parser.add_argument('--alpha', type=float, default=0.2, help='Alpha for the leaky_relu.')   # best: 0.2
...
def main():
    ...
    if opt.validation: # 注意:validation时的test_data是经过+1的; test时的test_data就是原本的id
        for sequence in seq:
            # 取倒数两个之前的列表作为训练集
        ...
        model=torch.load('./my_data/Amazon_'+opt.dataset+'/'+opt.model)
        model.cuda()
        for sequence in seq:
            # 取倒数两个之前的列表作为训练集
            input_ids = sequence
