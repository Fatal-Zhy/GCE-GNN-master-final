...
def train_test(model, train_data, test_data,test_data_panduan,is_val):
    if not is_val:
        model.eval()
        ...
        for data in test_loader:
            ...
            sub_scores = scores.topk(30)[1]  ###############
            # sub_scores=sub_scores[:,1:]
            # print(sub_scores)
            sub_scores = trans_to_cpu(sub_scores).detach().numpy()
            targets = targets.numpy()
            for score, target, mask in zip(sub_scores, targets, test_data.mask):
                # print(test_data_panduan[i],score)
                # print('++++++++++')
                for ii in test_data_panduan[i]:
                    if ii in score:
                        score = np.delete(score, list(score).index(ii))
                score = score[:20]
                preds.append(score)
                i+=1
        ...
        result.append(0)
        result.append(0)
        return result[0], result[1], preds
    ...
    for data in test_loader:
        ...
        for score, target, mask in zip(sub_scores, targets, test_data.mask):
            # print(test_data_panduan[i],score)
            # print('++++++++++')
            # if 0 in score: # score是包含0的
            #     print('==============')
            for ii in test_data_panduan[i]:
                # if 0==ii: # 没有0
                #     print('+++++++++')
                if ii-1 in score: ##########################
                    score = np.delete(score, list(score).index(ii-1))
            i+=1
            score=score[:20]
            preds.append(score)
            hit.append(np.isin(target - 1, score))
            # print(score,type(score),score.shape)
            # print(target)
            # print('--------------')
            if len(np.where(score == target - 1)[0]) == 0: # 如果这个score不包含target-1,就相当于没预测到
                mrr.append(0)
            else:
